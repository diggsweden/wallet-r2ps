...
impl DeviceMetadataService {
    pub fn new(client_repository_spi_port: Arc<dyn ClientRepositorySpiPort + Send + Sync>,
               device_permit_list_spi_port: Arc<dyn DevicePermitListSpiPort + Send + Sync>,) -> Self {
    ...
    pub fn update_device_permit_list(&self, device_id: DeviceId, permit_list_dto: PermitListDto) -> Result<(), PermitListError> {
        self.device_permit_list_spi_port.store_permit_list(device_id, permit_list_dto).map_err(|e| e.into())
    }
}
