...
impl R2psService {
    ...
    pub fn encrypt_with_aes(&self,
        payload: &Vec<u8>, pake_session_id: &str
        ...
        match self.session_key_spi_port.get(pake_session_id) {
            Some(session_key) => {
                ...
                jwe::serialize_compact(payload, &header, &encrypter)
                    .map_err(|e| {
    ...
    pub fn decrypt_jwe(&self, encrypted_payload: &str, pake_session_id: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        match self.session_key_spi_port.get(pake_session_id) {
            Some(session_key) => {
                    ...
                    Ok(data) => {
                        ...
                        match String::from_utf8(data) {
                            Ok(decoded_string) => {
                                ...
                                let (payload, _header) = josekit::jwe::deserialize_compact(&decoded_string, &decrypter)?;
                                Ok(payload)
                            },
                            Err(_) => Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "No session key")))
                        }
                    },
                    Err(_) => Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "No session key")))
                }
            },
            None => Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "No session key")))
        }
    }
    ...
    pub fn decode_r2ps_request_jws(&self, input: &R2psRequest, client_public_key: &Pem) -> Result<ServiceRequest, ServiceRequestError>{
...
impl R2psRequestUseCase for R2psService {
    ...
    fn execute(&self, r2ps_request: R2psRequest) -> Result<R2psRequestId, R2psRequestError> {
        ...
        let service_request = self.decode_r2ps_request_jws(&r2ps_request, &client_metadata.client_public_key)
        ...
        let decrypted_payload = match service_request.service_type.encrypt_option() {
            EncryptOption::User => self.decrypt_jwe(&service_request.clone().service_data.unwrap(), &service_request.clone().pake_session_id.unwrap()).map_err(|_| R2psRequestError::DecryptionError)?,
        ...
        let response = self.process_service_request(
            &service_request,
            &decrypted_payload,
        ...
        let jwe = match service_request.service_type.encrypt_option() {
            EncryptOption::User => {
                info!("user encrypted, aes encrypt response data with session key: {:?}", response);
        ...
        info!("JWS response payload on {:?} {}", service_request.service_type,  jws);
                                            ...
                                            match josekit::jwe::deserialize_compact(&decoded_string, &decrypter) {
                                                Ok((payload, header)) => {
                                                    ...
                                                    match String::from_utf8(payload.clone()) {
                                                        Ok(decrypted_text) => info!("Decrypted text: {}", decrypted_text),
                                                        Err(error) => warn!("Error decrypting JWS payload: {:?}", error),
                                                    };
                                                    Ok(payload.to_vec())
                                                },
