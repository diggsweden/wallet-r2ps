...
impl DevicePermitListSpiPort for DevicePermitListMemoryCache {
    fn store_permit_list(&self, device_id: DeviceId, permit_list_item: PermitListDto) -> Result<(), PermitListError> {
        info!("storing item to permit list device_id: {}", device_id);
        self.permit_list.insert(device_id.to_string(), permit_list_item.clone());
        Ok(())
    }
