...
def generate_summary(src_dir: Path, api_ref_entries: List[Tuple[str, str]]) -> str:
    """Generate SUMMARY.md content.

    Discovers hand-written guide pages from *src_dir* (any .md file that is
    not SUMMARY.md, introduction.md, or inside api-reference/) and combines
    them with the generated *api_ref_entries*.

    Args:
        src_dir: The mdbook ``src`` directory.
        api_ref_entries: List of (display_title, filename) for each generated
            API reference page, in the order they should appear.

    Returns:
        The full SUMMARY.md content as a string.
    """
    # Map of filename stem to a nice title – derive from the H1 heading
    # inside the file when possible, otherwise title-case the stem.
    guide_pages: List[Tuple[str, str]] = []  # (title, filename)
    skip = {"SUMMARY.md", "introduction.md"}
    for md in sorted(src_dir.glob("*.md")):
        if md.name in skip:
            continue
        # Try to read the first H1 heading
        ...
        try:
            ...
            pass
        guide_pages.append((title, md.name))
    ...
    if guide_pages:
        lines.append("# Guide")
        lines.append("")
        for title, filename in guide_pages:
            lines.append(f"- [{title}]({filename})")
        lines.append("")

    lines.append("# API Reference")
    lines.append("")
    lines.append("- [API Reference](api-reference/README.md)")
    for title, filename in api_ref_entries:
        lines.append(f"  - [{title}](api-reference/{filename})")
...
def main():
    ...
    # Collect entries for SUMMARY.md generation
    api_ref_entries: List[Tuple[str, str]] = []  # (display_title, filename)
...
"""
        ...
        # Collect entry for SUMMARY.md
        if is_wrapper_type(type_name):
            display_title = get_wrapper_title(type_name).replace("<", "\\<").replace(">", "\\>")
        else:
            display_title = type_name
        api_ref_entries.append((display_title, filename))

        print(f"Generated {output_path}")

    # Generate SUMMARY.md
    src_dir = output_dir.parent  # api-reference's parent is src/
    summary_content = generate_summary(src_dir, api_ref_entries)
    summary_path = src_dir / "SUMMARY.md"
    with open(summary_path, 'w') as f:
        f.write(summary_content)
    print(f"Generated {summary_path}")

    print(f"\nSuccessfully generated {len(all_types_to_doc)} type documentation files + SUMMARY.md")
