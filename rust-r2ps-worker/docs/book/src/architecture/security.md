...
The redb cache is kept up-to-date across all worker instances via the `state-snapshot` Kafka topic. Every worker independently consumes all messages (no consumer group) so that each has a complete view of all device versions cluster-wide. See [State Management — State-snapshot consumer](state-management.md#state-snapshot-consumer) for the full design.
