...
usage() {
  ...
  echo "                         If not set, this location defaults to /usr/lib/aarch64-linux-gnu/engines-3/libpkcs11.so"
  echo
}
...
LIBPKCS11_LOCATION="/usr/lib/aarch64-linux-gnu/engines-3/libpkcs11.so"
...
if ! [ $((${#KID} % 2)) -eq 0 ]; then
  echo "Key ID $KID has odd number of digits. Aborting" >&2
  exit 1
fi
...
cat <<EOF >/tmp/target/p11-openssl.cfg
...
pkcs11-tool --module $MODULE --slot $SLOT --login -p ${PASSWD} --keypairgen --id $((KID)) --label $ALIAS --key-type $KEY_TYPE --extractable
...
OPENSSL_CONF=/tmp/target/p11-openssl.cfg openssl req -x509 -new \
  -engine pkcs11 \
  -keyform engine \
  -key ${KEY_ID} \
  -passin "pass:${PASSWD}" \
  -${HASH} \
  -config /tmp/target/cert.cfg \
  -subj "${DN}" \
  -days ${VALID_DAYS} \
  -out "${ALIAS}.crt"
