...
public class R2psResponseValKeySink implements R2psResponseSinkSpiPort {
  ...
  public R2psResponseValKeySink(Config config,
  ...
  public void storeResponse(R2psResponse r2psResponse) {
    redisTemplate.opsForValue().set(r2psResponse.correlationId(), r2psResponse,
        config.getKafka().rest().responseTtlSeconds(),
        TimeUnit.SECONDS);
  }

  @Override
  public Optional<R2psResponse> loadResponse(String correlationId) {
    return Optional.ofNullable(redisTemplate.opsForValue().get(correlationId));
  }
}
